...
export const Route = createFileRoute("/stays")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Bespoke stays in a peaceful Norfolk farmhouse. Solo guests, couples, families and small groups welcome." },
      { property: "og:title", content: "Stays — Home House Homestead" },
      { property: "og:description", content: "Bespoke stays in a peaceful Norfolk farmhouse." },
      { property: "og:image", content: IMG.hero },
      { property: "og:url", content: "/stays" },
    ],
    links: [{ rel: "canonical", href: "/stays" }],
  }),
  component: StaysPage,
});
      ...
      <Section className="border-t border-border">
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <ul className="reveal grid gap-3">
            {includes.map((i) => (
              <li key={i} className="border-b border-border pb-3 text-foreground/85 font-light">{i}</li>
            ))}
          </ul>
        </div>
